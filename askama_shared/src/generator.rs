...
impl<'a, S: std::hash::BuildHasher> Generator<'a, S> {
    ...
    fn impl_actix_web_responder(&mut self, buf: &mut Buffer) -> Result<(), CompileError> {
        ...
        //buf.writeln("type Future = ::askama_actix::futures::Ready<::std::result::Result<::actix_web::BaseHttpResponse, ::actix_web::Error>>;")?;
        ...
        buf.writeln("self.to_response().expect(\"Failed to generate response\")")?;
